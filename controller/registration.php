...
if (isset($_POST["submit"])) {
    ...
    if (!empty($firstname) && !empty($lastname) && !empty($email) && !empty($password)) {
        ...
        if ($rowcount > 0) {
            ...
            if (!preg_match("/^(?=.*\d)(?=.*[@#\-_$%^&+=§!\?])(?=.*[a-z])(?=.*[A-Z])[0-9A-Za-z@#\-_$%^&+=§!\?]{6,13}$/", $password)) {
                $password_Err = '<div class = "alert alert-danger">
                Password should be between 6 to 13 characters long, contains at least one special character, lowercase, uppercase and a digit.
                </div>';
            }
